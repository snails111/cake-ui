...
export default class SearchForm extends React.Component {
	...
	getField = (item = {}) => {
		...
		const FormItemContainer = ({ ...props }) => {
			...
			return (
				<Col
					...
					style={{
						display:
							this.totalColSpan <= 24 * collapsedShowRow - buttonsColSpan || this.state.expand
								? "block"
								: "none",
					}}
				>
		...
		switch (item.type) {
				...
				return (
					<FormItemContainer item={item}>
						...
						>
							{item.options &&
								item.options.map(op => (
									<Option {...op} key={op.value} value={op.value}>
										{op.label}
									</Option>
								))}
						</Select>
							...
							{item.options &&
								item.options.map(op => (
									<TreeNode key={op.id} selectable={false} title={op.name} value={op.name}>
										{op.member &&
											op.member.map(subItem => {
												return (
													<TreeNode key={subItem.id} title={subItem.name} value={subItem.id} />
												);
											})}
									</TreeNode>
								))}
			...
			case "custom":
				return <FormItemContainer item={item}>{item.render(this.formRef)}</FormItemContainer>;
			default:
				return null;
		}
	};
