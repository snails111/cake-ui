...
export default class MyForm extends React.Component {
			...
			rules.map(item => {
				...
				if (item.validator) {
					obj.validator = (rule, value, callback) =>
						item.validator(rule, value, callback, this.props.form);
					newRules.push({ ...obj });
				} else {
					newRules.push({ ...item });
				}
	...
	getField = item => {
		switch (item.type) {
					...
					<Search
						...
						onSearch={(value, e) => {
							...
							if (e.target.tagName === "BUTTON") {
								item.onSearch &&
									item.onSearch(value, this.formRef, () => {
										// (发送验证码)请求成功回调
			...
			case "radio":
				return (
					...
					>
						{item.options &&
							item.options.map(op => (
								<Radio key={op.value} disabled={item.disabled} value={op.value}>
									{op.label}
								</Radio>
							))}
					</RadioGroup>
			...
			case "file":
				return <MyUpload {...item} form={this.formRef} />;
			// 自定义组件
			case "custom":
				return item.render(this.formRef);
			default:
				return null;
		}
	};
		...
		return (
			...
			>
				...
				<div className="action-button-container" {...buttonConfig}>
					{buttonConfig && buttonConfig.beforeCustomChildren
						? buttonConfig.beforeCustomChildren()
						: null}
					{buttonConfig && buttonConfig.showCancelBtn ? (
						<Button onClick={() => onCancel && onCancel()}>{cancelText || "取消"}</Button>
					) : null}
					{buttonConfig && buttonConfig.showOkBtn ? (
						<Button htmlType="submit" type="primary" style={{ marginLeft: 16 }}>
							{okText || "确定"}
						</Button>
					) : null}
					{buttonConfig && buttonConfig.afterCustomChildren
						? buttonConfig.afterCustomChildren()
						: null}
				</div>
			</Form>
		);
